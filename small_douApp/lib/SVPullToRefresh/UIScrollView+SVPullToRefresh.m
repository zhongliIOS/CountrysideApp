...
@interface SVPullToRefreshCustomActivityIndicatorView : UIView
{
    NSTimer *_timer;
}
...
@property (nonatomic, strong) SVPullToRefreshCustomActivityIndicatorView *activityIndicatorView;
...
- (SVPullToRefreshCustomActivityIndicatorView *)activityIndicatorView {
    if(!_activityIndicatorView) {
        _activityIndicatorView = [[SVPullToRefreshCustomActivityIndicatorView alloc]init];
        [self addSubview:_activityIndicatorView];
    }
    return _activityIndicatorView;
}
...
@implementation SVPullToRefreshCustomActivityIndicatorView
...
- (void)startAnimating{
    ...
    _imageview.transform = CGAffineTransformIdentity;
    [self round:nil];
    _timer = [NSTimer scheduledTimerWithTimeInterval:0.01 target:self selector:@selector(round:) userInfo:nil repeats:YES];
}
- (void)round:(NSTimer *)timer{
    
        [UIView animateWithDuration:0.01 animations:^{
            _imageview.transform = CGAffineTransformRotate(_imageview.transform,0.05);
            _imageview.transform = CGAffineTransformRotate(_imageview.transform,0.05);
